...
let curLoadChartView = 'avg_load'; // 'avg_load' or 'session_distribution'
let currentBlockDistribution = typeof DEFAULT_BLOCK_DISTRIBUTION !== 'undefined' ? JSON.parse(JSON.stringify(DEFAULT_BLOCK_DISTRIBUTION)) : {};
let currentSiteRecords = (typeof DATA !== 'undefined' && DATA.session_sites) ? JSON.parse(JSON.stringify(DATA.session_sites)) : []; // Individual session site records from default data or uploaded CSV

// Delivery Point chart state variables
let deliveryDataset = [];
let curDeliverySort = 'delivery_desc';
...
function processRawRows(rows, fileName) {
  ...
  try {
    rows.forEach(row => {
      ...
      // Bin vaccination counts by block for session sites
      if (!currentBlockDistribution[subDistrict]) {
        currentBlockDistribution[subDistrict] = {
          pw: [0, 0, 0],
          inf: [0, 0, 0],
          ch: [0, 0, 0]
        };
      }
      let blockDist = currentBlockDistribution[subDistrict];
      if (pw === 0) blockDist.pw[0]++;
      else if (pw >= 1 && pw <= 5) blockDist.pw[1]++;
      else if (pw >= 6) blockDist.pw[2]++;

      if (inf === 0) blockDist.inf[0]++;
      else if (inf >= 1 && inf <= 5) blockDist.inf[1]++;
      else if (inf >= 6) blockDist.inf[2]++;

      if (ch === 0) blockDist.ch[0]++;
      else if (ch >= 1 && ch <= 5) blockDist.ch[1]++;
      else if (ch >= 6) blockDist.ch[2]++;

      // Store individual session site record for the distribution chart
      const siteLabel = (siteName || hscName || facilityName || '').trim();
      if (siteLabel) {
        currentSiteRecords.push({
          name: siteLabel,
...
function resetToDefaultData() {
  ...
  currentBlockDistribution = typeof DEFAULT_BLOCK_DISTRIBUTION !== 'undefined' ? JSON.parse(JSON.stringify(DEFAULT_BLOCK_DISTRIBUTION)) : {};
  currentSiteRecords = defaultDataObj.session_sites ? JSON.parse(JSON.stringify(defaultDataObj.session_sites)) : []; // Reset site records to default data
...
function changeViewLevel(lvl) {
  ...
  const loadSuffix = document.getElementById('chart-load-level-suffix');
  if (loadSuffix) {
    loadSuffix.textContent = `(${lvl === 'facility' ? 'by facility' : 'by HSC'})`;
    loadSuffix.style.display = curLoadChartView === 'avg_load' ? '' : 'none';
  }
  ...
  const distAvgEl = document.getElementById('district-avg-label');
  if (distAvgEl) distAvgEl.textContent = `District avg: ${distAvg}`;
...
function renderTable() {
  ...
  if (!data.length) {
    tbody.innerHTML = `<tr><td colspan="13" class="no-data"><i class="ti ti-ban" style="font-size:24px; display:block; margin-bottom:8px; color:var(--color-text-tertiary)"></i>No items match the search criteria</td></tr>`;
...
function renderKPIs() {
  ...
  const tot_pw = DATA.facilities.reduce((s, f) => s + (f.pw_vacc || 0), 0);
  const tot_inf = DATA.facilities.reduce((s, f) => s + (f.infants || 0), 0);
  const tot_ch = DATA.facilities.reduce((s, f) => s + (f.children_gt1 || 0), 0);
  const pDO = tot_p1 > 0 ? ((tot_p1 - tot_p3) / tot_p1 * 100).toFixed(1) : "0.0";

  // Build beneficiary card HTML (single combined card)
  function beneRow(label, val, color, pct) {
    return `<div style="display:flex;justify-content:space-between;align-items:center;padding:3px 0;border-bottom:1px solid var(--color-border-primary);">
          <span style="font-size:11px;font-weight:600;color:${color};">${label}</span>
          <span style="font-size:12px;font-weight:700;">${val.toLocaleString()} <span style="font-size:10px;color:var(--color-text-tertiary);font-weight:600;">(${pct}%)</span></span>
        </div>`;
  }

  const beneCard = tot_bene > 0 ? `
        <div class="kpi" style="flex-direction:column;align-items:flex-start;gap:6px;min-width:220px;">
          <div class="kpi-lbl" style="margin-bottom:2px;">Beneficiary Breakdown</div>
          ${beneRow('Pregnant Women', tot_pw, '#ec4899', (tot_pw / tot_bene * 100).toFixed(1))}
          ${beneRow('Infants (0–1 yr)', tot_inf, '#3b82f6', (tot_inf / tot_bene * 100).toFixed(1))}
          ${beneRow('Children (>1 yr)', tot_ch, '#10b981', (tot_ch / tot_bene * 100).toFixed(1))}
          <div style="width:100%;height:8px;border-radius:99px;overflow:hidden;display:flex;margin-top:2px;">
            <div style="width:${(tot_pw / tot_bene * 100).toFixed(1)}%;background:#ec4899;" title="Pregnant Women"></div>
            <div style="width:${(tot_inf / tot_bene * 100).toFixed(1)}%;background:#3b82f6;" title="Infants"></div>
            <div style="width:${(tot_ch / tot_bene * 100).toFixed(1)}%;background:#10b981;" title="Children"></div>
          </div>
          <div class="kpi-sub">District total: ${tot_bene.toLocaleString()} beneficiaries</div>
        </div>` : '';
...
function renderCharts() {
  ...
  const loadSuffix = document.getElementById('chart-load-level-suffix');
  if (loadSuffix) {
    loadSuffix.textContent = `(by ${isHscDataForCharts ? 'HSC' : 'facility'})`;
    loadSuffix.style.display = curLoadChartView === 'avg_load' ? '' : 'none';
  }
  ...
  const loadLegendEl = document.getElementById('load-chart-legend');
  if (loadLegendEl) {
    if (curLoadChartView === 'avg_load') {
      loadLegendEl.innerHTML = `
            <span><span class="ld" style="background:#6366f1"></span>Avg/session</span>
            <span style="color:var(--color-text-tertiary); font-size:11px; margin-left:auto;" id="district-avg-label">District avg: ${computedDistAvg}</span>
          `;
    } else {
      loadLegendEl.innerHTML = `
            <span><span class="ld" style="background:#ef4444"></span>0 Vaccinations</span>
            <span><span class="ld" style="background:#f59e0b"></span>1 to 5 Vaccinations</span>
            <span><span class="ld" style="background:#10b981"></span>6 or More Vaccinations</span>
            <span style="color:var(--color-text-tertiary); font-size:11px; margin-left:auto;">${currentSiteRecords.length > 0 ? 'Session Sites' : 'Facilities'}</span>
          `;
    }
  }
  ...
  // 4. Session Load Chart (or Beneficiary Distribution Chart)
  if (curLoadChartView === 'avg_load') {
    ...
    if (loadWrapper) {
      ...
      const dynamicWidth = Math.max(parentWidth, sortedLoad.length * 35);
      loadWrapper.style.width = `${dynamicWidth}px`;
      loadWrapper.style.height = '';
    }
    ...
    // Session site vaccination distribution – stacked bar chart by facility, showing session counts by bucket (0, 1-5, 6+)
    let facilityData = [];

    // Group session sites by facility
    const facilityMap = {};
    
    // Use individual session site records (always populated from DATA.session_sites)
    let records = blk
      ? currentSiteRecords.filter(s => s.block === blk)
      : currentSiteRecords;

    records.forEach(s => {
      const fac = s.facility || "Other/Unassigned";
      if (!facilityMap[fac]) {
        facilityMap[fac] = {
          name: fac,
          block: s.block,
      ...
      const val = s.total;
      const bucket = val === 0 ? 0 : val <= 5 ? 1 : 2;
      if (bucket === 0) {
        facilityMap[fac].bucket0++;
        facilityMap[fac].sites0.push(s.name);
      } else if (bucket === 1) {
        facilityMap[fac].bucket1++;
        facilityMap[fac].sites1.push(s.name);
      } else {
        facilityMap[fac].bucket2++;
        facilityMap[fac].sites2.push(s.name);
      }
    });

    facilityData = Object.values(facilityMap);

    // Sort facilities alphabetically by name
    facilityData.sort((a, b) => a.name.localeCompare(b.name));

    // Filter by category selection checkboxes
    const showBucket0 = document.getElementById('chk-bucket0')?.checked ?? true;
    const showBucket1 = document.getElementById('chk-bucket1')?.checked ?? true;
    const showBucket2 = document.getElementById('chk-bucket2')?.checked ?? true;

    // Apply filters to stack values
    const displayData = facilityData.map(f => {
      return {
        name: f.name,
        block: f.block,
        bucket0: showBucket0 ? f.bucket0 : 0,
        bucket1: showBucket1 ? f.bucket1 : 0,
        bucket2: showBucket2 ? f.bucket2 : 0,
        sites0: f.sites0,
        sites1: f.sites1,
        sites2: f.sites2
      };
    ...
    const bucketColors = ['#ef4444', '#f59e0b', '#10b981'];

    // Update dynamic custom legend in HTML with correct count & status
    const loadLegendEl = document.getElementById('load-chart-legend');
    if (loadLegendEl) {
      loadLegendEl.innerHTML = `
            <span><span class="ld" style="background:#ef4444"></span>0 Vaccinations</span>
            <span><span class="ld" style="background:#f59e0b"></span>1 to 5 Vaccinations</span>
            <span><span class="ld" style="background:#10b981"></span>6 or More Vaccinations</span>
            <span style="color:var(--color-text-tertiary); font-size:11px; margin-left:auto; font-weight:600;">Showing ${displayData.length} Facilities</span>
          `;
    }

    // Dynamically size the chart wrapper for vertical stacked bar chart with horizontal scrolling
    if (loadWrapper) {
      const parentWidth = loadWrapper.parentElement.clientWidth || 600;
      const dynamicWidth = Math.max(parentWidth, displayData.length * 60); // 60px per facility
      loadWrapper.style.width = `${dynamicWidth}px`;
      loadWrapper.style.height = '';
    }

    const shortFacName = name => {
      if (!name) return "";
      return name
        .replace(/ CHC| PHC| UPHC| HSC/g, '')
        .replace('LNJPN SADAR HOSPITAL BHAGALPUR', 'LNJPN Sadar')
        .replace('Jawaharlal Nehru Medical College and Hospital  Bgp', 'JLNMCH')
        .substring(0, 15);
    };
    ...
    chartLoad = new Chart(document.getElementById('c-load'), {
      type: 'bar',
      data: {
        labels: displayData.map(f => shortFacName(f.name)),
        datasets: [
          {
            label: '0 Vaccinations',
            data: displayData.map(f => f.bucket0),
            backgroundColor: bucketColors[0],
            borderRadius: 3
          },
          {
            label: '1 to 5 Vaccinations',
            data: displayData.map(f => f.bucket1),
            backgroundColor: bucketColors[1],
            borderRadius: 3
          },
          {
            label: '6 or More Vaccinations',
            data: displayData.map(f => f.bucket2),
      ...
      options: {
          ...
          tooltip: {
            callbacks: {
              title: (items) => {
                ...
                return displayData[idx] ? displayData[idx].name : items[0].label;
              },
              label: (item) => {
                const f = displayData[item.dataIndex];
                if (!f) return "";
                const datasetIndex = item.datasetIndex; // 0: 0-vacc, 1: 1-5, 2: 6+
                let count = 0;
                let categoryLabel = "";
                if (datasetIndex === 0) {
                  count = f.bucket0;
                  categoryLabel = "0 Vaccinations";
                } else if (datasetIndex === 1) {
                  count = f.bucket1;
                  categoryLabel = "1 to 5 Vaccinations";
                } else {
                  count = f.bucket2;
                  categoryLabel = "6 or More Vaccinations";
                }
                return `${categoryLabel}: ${count} session site${count !== 1 ? 's' : ''}`;
              },
              afterBody: (items) => {
                const f = displayData[items[0].dataIndex];
                if (!f) return [];
                const datasetIndex = items[0].datasetIndex;
                let sitesList = [];
                if (datasetIndex === 0) sitesList = f.sites0;
                else if (datasetIndex === 1) sitesList = f.sites1;
                else sitesList = f.sites2;

                if (sitesList.length === 0) return [];
                
                const maxToShow = 6;
                let sitesStr = sitesList.slice(0, maxToShow).join(', ');
                if (sitesList.length > maxToShow) {
                  sitesStr += ` and ${sitesList.length - maxToShow} more...`;
                }
                return [`\nSession Sites:`, sitesStr];
              }
            }
          },
          datalabels: {
        ...
        scales: {
          x: { stacked: true, ticks: { font: { size: 9 }, color: '#64748b', maxRotation: 45, autoSkip: false }, grid: { display: false } },
          y: { stacked: true, grace: '12%', ticks: { font: { size: 10 }, color: '#64748b' }, grid: { color: '#f1f5f9' } }
        }
      }
    });
  }
...
function onLoadChartViewChange() {
  const select = document.getElementById('load-chart-view-select');
  if (select) {
    curLoadChartView = select.value;
  }
  const filtersEl = document.getElementById('session-dist-filters');
  if (filtersEl) {
    filtersEl.style.display = curLoadChartView === 'session_distribution' ? 'flex' : 'none';
  }
  renderCharts();
}
window.onLoadChartViewChange = onLoadChartViewChange;

// ── Delivery Point Performance Chart ─────────────────────────────────────

// Parse delivery CSV data (from default constant or uploaded file)
function parseDeliveryCSV(csvString) {
  const results = Papa.parse(csvString.trim(), {
    header: true,
    dynamicTyping: true,
    skipEmptyLines: true
  });
  return processDeliveryRows(results.data);
}

function processDeliveryRows(rows) {
  const parsed = [];
  rows.forEach(row => {
    const dpName = getVal(row, ['Delivery Point Name', 'deliverypointname'], false);
    if (!dpName) return;

    // Determine block name: use LGD Block Name, or Sub District name as fallback
    let block = getVal(row, ['LGD Block Name', 'lgdblockname'], false);
    if (!block) block = getVal(row, ['Sub District name', 'Sub District', 'subdistrict', 'subdistrictname'], false);
    // Normalize block name to title case for consistent matching
    if (block) {
      block = block.charAt(0).toUpperCase() + block.slice(1).toLowerCase();
      // Special handling for multi-word blocks
      block = block.replace(/\b\w/g, c => c.toUpperCase());
    }

    const delivery = getVal(row, ['Number of Pregnancy Outcome Registered - Delivery', 'pregnancyoutcomeregistereddelivery']);
    const liveBirth = getVal(row, ['Number of New Born Registered - Live Birth', 'newbornregisteredlivebirth']);
    const birthDoses = getVal(row, ['Number of New Born for which Birth doses updated', 'newbornbirthDosesupdated']);
    ...
    parsed.push({
      ...
      delivery: delivery,
      liveBirth: liveBirth,
      birthDoses: birthDoses
    });
  });
  return parsed;
}

function onDeliverySortChange() {
  const select = document.getElementById('delivery-sort-select');
  if (select) {
    curDeliverySort = select.value;
  }
  renderDeliveryChart();
}
window.onDeliverySortChange = onDeliverySortChange;

function renderDeliveryChart() {
  if (chartDelivery) chartDelivery.destroy();
  chartDelivery = null;

  const canvas = document.getElementById('c-delivery');
  if (!canvas || !deliveryDataset || deliveryDataset.length === 0) return;

  // Apply block filter
  const blk = document.getElementById('block-filter').value;
  let filtered = deliveryDataset;
  if (blk) {
    // Match block case-insensitively
    filtered = deliveryDataset.filter(d => d.block.toLowerCase() === blk.toLowerCase());
  }
  ...
  if (curDeliverySort === 'delivery_desc') {
    sorted.sort((a, b) => b.delivery - a.delivery);
  } else if (curDeliverySort === 'live_birth_desc') {
    sorted.sort((a, b) => b.liveBirth - a.liveBirth);
  } else if (curDeliverySort === 'doses_desc') {
    sorted.sort((a, b) => b.birthDoses - a.birthDoses);
  } else if (curDeliverySort === 'name_asc') {
    sorted.sort((a, b) => a.name.localeCompare(b.name));
  }

  // Build labels: "Delivery Point (Block)"
  const labels = sorted.map(d => {
    ...
    return `${shortName} (${d.block})`;
  });

  // Size wrapper dynamically
  const wrapper = document.getElementById('c-delivery-wrapper');
  if (wrapper) {
    const parentWidth = wrapper.parentElement.clientWidth || 600;
    const dynamicWidth = Math.max(parentWidth, sorted.length * 70);
    wrapper.style.width = `${dynamicWidth}px`;
  }

  chartDelivery = new Chart(canvas, {
    type: 'bar',
    data: {
      labels: labels,
      datasets: [
        ...
        {
          label: 'Birth Doses Updated',
          data: sorted.map(d => d.birthDoses),
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            title: (items) => {
              const idx = items[0].dataIndex;
              return sorted[idx] ? `${sorted[idx].name} (${sorted[idx].block})` : items[0].label;
            }
          }
        },
        datalabels: {
      ...
      scales: {
        x: {
          ticks: { font: { size: 9 }, color: '#64748b', maxRotation: 55, autoSkip: false },
          grid: { display: false }
        },
        y: {
          grace: '12%',
...
window.renderDeliveryChart = renderDeliveryChart;

// Initialize delivery data from default CSV
function initDeliveryData() {
  if (typeof DEFAULT_DELIVERY_CSV !== 'undefined' && DEFAULT_DELIVERY_CSV) {
    deliveryDataset = parseDeliveryCSV(DEFAULT_DELIVERY_CSV);
    renderDeliveryChart();
  }
}
initDeliveryData();
